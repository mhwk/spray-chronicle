    ...
    public class RavenProcessingPipelineTest : RavenTestCase
    {
        ...
        public async Task TestSavesNewDocument()
        {
            ...
            var pipeline = Container()
                .Resolve<RavenProcessingPipeline<QueryBasketWithProducts, BasketWithProducts_v2>>();
            ...
            using (var session = store.OpenAsyncSession()) {
                var result = await session.LoadAsync<BasketWithProducts_v2>(identity1);
...
//            await Task.Delay(TimeSpan.FromSeconds(.3));
        ...
        {
            builder.RegisterQueryExecutor<QueryBasketWithProducts,BasketWithProducts_v2>("foo", _checkpointName);
        }
    }
}
